...
- Functions for the most important http verbs:
  ...
  [`DELETE()`](https://httr.r-lib.org/reference/DELETE.md) and
...
- Requests return a standard reponse object that captures the http
  ...
  - Response content is available with
    [`content()`](https://httr.r-lib.org/reference/content.md) as a raw
    vector (`as = "raw"`), a character vector (`as = "text"`), or parsed
    into an R object (`as = "parsed"`), currently for html, xml, json,
    png and jpeg.
...
- Config functions make it easier to modify the request in common ways:
  ...
  [`progress()`](https://httr.r-lib.org/reference/progress.md).
