...
backoff_full_jitter <- function(i, resp, pause_base = 1, pause_cap = 60, pause_min = 1,
                                quiet = FALSE) {
  ...
  if (!quiet) {
    ...
    message(error_description, "Request failed [", status, "]. Retrying in ", round(length, 1L), " seconds...")
  }
  Sys.sleep(length)
}
