...
#' @export
http_status <- function(x) UseMethod("http_status")

#' @export
http_status.reponse <- function(x) http_status(x$status_code)

#' @export
http_status.numeric <- function(x) {

  status_desc <- http_statuses[as.character(x)]  
  if (is.na(status_desc)) {
    stop("Unknown http status code: ", x)
  }

  status_types <- c("information", "success", "redirection", "client error", 
    "server error")
  status_type <- status_types[x %/% 100]

  # create the final information message
  message <- paste(status_type, ": (", x, ") ", status_desc, sep = "")
