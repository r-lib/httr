...
This document collects best practices for writing a package that connects to a web API. The goal is to help you produce a package that is safe, secure and keeps working in the long run.
...
Including authentication requiring code in your vignettes is challenging, because you don't want to bundle your own credentials with the package! However, you can take advantage of the fact that the vignette is built locally, and only checked by CRAN with this idea from Jenny Bryan:
...
1.  Create a personal access token in the 
    [Personal access tokens area](https://github.com/settings/tokens) of your
    GitHub personal settings. Copy token to the clipboard.
...
1.  Create a line like this:
    ...
    where the name `GITHUB_PAT` reminds you which API this is for and 
    `blahblahblahblahblahblah` is your personal access token, pasted from the clipboard.
