...
An API package needs to be able to generate these components in order to authenticate, and perform the desired API call. For example, to request that the GitHub API provides a list of all issues for the httr repo, we send an HTTP request that looks like:
...
In response to this request, the API will return some form of response that includes:

1. An HTTP status code
1. Headers
1. A response body typically consisting of XML, JSON, plain text, HTML, 
   or some kind of binary representation. 
   
An API client needs to parse these responses, turn API errors into R errors, and return a useful object to the end user. For the previous HTTP request, GitHub returns:
...
It is best to start with some helper functions to capture the common conventions of the API in one place. This allows some someone who is familiar with the API (i.e. you), to generate HTTP requests and turn the response into a useful R object.

These functions should capture information like:

- the base URL for all API requests.
- how data passed to the API
...
Next, you need to take the response returned by the API and turn it into a useful object. Any API will return an HTTP response that consists of headers and a body. While the response can come in a multiple a forms (see above), two of the most common structured formats are XML and JSON. Most APIs will return only one or the other, some, like the colour lovers API, allow you to choose which one with a url parameter:
...
Next, you need to make sure that your API wrapper throws an error if the request failed.

Using a web API introduces additional possible points of failure into R code aside from those occuring in R itself. These include:

- Client-side exceptions
- Network / communication exceptions
- Server-side exceptions

You can do this by checking the `status_code()`: if it's 200, the request succeeded, otherwise it failed and you need to throw an error. Often the API will information about the error in the body of the response: you should use this where available.
...
Once you have working low-level wrappers for the API, you simply need to write higher-level, user-visible functions that will implement the features of the API within R. Most APIs work by executing an HTTP method on a specified URL with or without some additional parameters. These parameters can be specified in a number of ways, including in the URL path, in URL query arguments, in HTTP headers, and in the request body itself.  These parameters can be controlled using httr functions:
...
NB: Some APIs describe their authentication processes inaccurately, so care needs to be taken to understand the true authentication mechanism regardless of the label used in the API docs.

It is possible to specify the key(s) or token(s) required for basic or OAuth authentication in a number of different ways (see Appendix for a detailed discussion). You may also need some way to preserve user credentials between function calls so that end users do not need to specify them each time. Here is an example of how to write a function that checks for the presence of a GitHub personal access token and errors otherwise:
...
Need to make sure that when parsing the body, you use `tryCatch()` or similar to gracefully respond to the server returning invalid responses. Note that it's not uncommon for the server to claim that the response is one type in the `Content-type` while actually being something else (e.g. the content type say that's it XML, but it's actually json)

### Rate limiting

Many APIs are rate limited, which means that you can only send a certain number of requests per hour. 

Often if your request is rate limited, the error message will tell you how long you should wait before performing another request. You might want to expose this to the user, or even include a wall to `Sys.sleep()` that waits long enough.
...
You should also provide a function that exposes the rate limiting information to the user. 
