...
`httr` provides a function that does this for you. To make your code more reliable in the presence of temporary issues like network outages or brief periods of server-side unavailability, consider using `RETRY()` in place of functions like `GET()`, `POST()`, etc.
