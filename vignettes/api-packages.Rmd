...
Encourage your users to store their important information once, rather than typing it into the console - it's easy to accidentally publish your `.Rhistory` and you don't want it to contain private data.

## Appendix

If your package supports an authentication workflow based on an API key or token, you can encourage users to store that in an environment variable. We illustrate this using the [`github` R package](https://github.com/cscheid/rgithub), which wraps the Github v3 API. Tailor this template to your API + package and include in `README.md` or a vignette.

  * Create a personal access token in the [Applications area](https://github.com/settings/applications) of your GitHub personal settings. Copy token to the clipboard.
  ...
  * Use `Sys.getenv()` to access your token. For example, here's how to use your `GITHUB_TOKEN` with the `github` package:
